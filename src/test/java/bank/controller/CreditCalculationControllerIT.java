...
class CreditCalculationControllerIT {
    ...
    @Test
    @DatabaseSetup("classpath:dbunit/credit-calculate-dataset.xml")
    @DatabaseTearDown("classpath:dbunit/credit-calculate-dataset.xml")
    void shouldCalculate() throws Exception {
        mockMvc.perform(put("/users/3/credits/3/creditCards/3/operation=CALCULATE")
    ...
    void shouldPAY() throws Exception {
        mockMvc.perform(put("/credits/4/creditCards/4/operation=PAY"))
                .andExpect(status().isOk())
